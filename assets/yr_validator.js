function Validator(formSelector) {
  ...
  const validatorRules = {
    ...
    email(value) {
      ...
      return regex.test(value) ? undefined : `Please fill this field as an email address!`;
    },
    confirmed(message) {
      return function (value) {
        return value === select('#password', formEle).value ? undefined : `Please check your ${message || 'input'}`;
      };
    },
    min(minValue) {
      return function (value) {
        return value.length >= minValue ? undefined : `Please fill at least ${minValue} characters!`;
      };
    },
    max(maxValue) {
      return function (value) {
        return value.length <= maxValue ? undefined : `Please fill at most ${maxValue} characters!`;
      };
    },
  };
  ...
  const handleValidate = (e, input = e.target) => {
    ...
    let errMes;
    let isErr = thisRules.some((ruleFunc) => (errMes = ruleFunc(e.target.value)));
  ...
  inputs.forEach((input) => {
    ...
    rules.forEach((rule) => {
      ...
      if (Array.isArray(formRules[input.name])) formRules[input.name].push(ruleFunc);
    ...
    if (isValid) {
      if (typeof this.onSubmit === 'function') {
        ...
        const values = [...enableInputs].reduce((values, input) => {
          switch (input.type) {
            case 'radio':
              values[input.name] = select(`input[name=${input.name}`, formEle).value;
              break;
            case 'checkbox':
